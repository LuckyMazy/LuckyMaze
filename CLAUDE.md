...
LuckyMaze is an IDPA school project: a physical/digital maze with a trained AI solver, and players bet via a mobile web app on which exit the AI will choose. **This repo is the .NET 10 backend** — auth, players, bets, game state. The AI solver, maze generator and physical hardware (Raspberry Pi, electromagnets, LEDs) live elsewhere; do **not** add LLM/chat/Semantic Kernel code here.
